...
function makePageForEpisodes(episodeList) {
  ...
  episodeList.forEach(e => {
    ...
    titleEl.innerHTML = `${e.name} - S${e.season
      .toString()
      .padStart(2, "0")}E${e.number.toString().padStart(2, "0")}`;
    ...
    //Select menu option els created & appended for each episode
    let optionEl = document.createElement("option");
    selectEl.appendChild(optionEl);
    optionEl.innerText = `${e.name} - S${e.season
      .toString()
      .padStart(2, "0")}E${e.number.toString().padStart(2, "0")}`;
  });
}
